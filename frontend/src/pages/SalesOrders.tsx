...
export default function SalesOrders() {
  ...
  const { salesOrders: orders, loading, error } = useSelector((state: RootState) => state.orders)
  ...
  const [customerOptions, setCustomerOptions] = useState<EntityOption[]>([])
  const [productOptions, setProductOptions] = useState<EntityOption[]>([])
  const [refreshTokens, setRefreshTokens] = useState({ customer: 0, product: 0 })
  ...
  const isFormValid = useMemo(() => {
    ...
    if (!objectIdRegex.test(String(formData.customerId).trim())) {
      return false
    }
  ...
  const handleSave = () => {
    ...
    if (!objectIdRegex.test(String(formData.customerId).trim())) {
      setValidationError('Invalid Customer ID')
      return
    }
    ...
    if (isUpdateMode) {
      ...
      console.log('[DEBUG DISPATCH] updateSalesOrder called with id:', orderId)
      dispatch(updateSalesOrder({ id: orderId, data: payload }))
    } else {
  ...
  const handleCreatedEntity = (entity: EntityOption) => {
    if (quickCreateEntity === 'customer') {
      setRefreshTokens((prev) => ({ ...prev, customer: prev.customer + 1 }))
      setFormData((prev) => ({ ...prev, customerId: entity.id }))
      return
    }
    ...
    <Box>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={2} mt={1}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ sm: 'center' }}>
              ...
              />
              <Button variant="outlined" onClick={() => setQuickCreateEntity('customer')}>+ Add New</Button>
            </Stack>
      ...
      {quickCreateEntity ? (
        <QuickCreateModal
          ...
          existingOptions={quickCreateEntity === 'customer' ? customerOptions : productOptions}
